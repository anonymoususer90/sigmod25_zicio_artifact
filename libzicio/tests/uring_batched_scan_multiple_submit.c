	...
	if (idx == 0) {
		...
		if (target_page_cnt < ud->nr_page) {
			...
			offset = ((unsigned long long)target_page_cnt -
					  accumulated_nr_pages[ud->cur_fd_idx]) * 2 * PAGE_SIZE;
			page_num_diff = ud->nr_page - target_page_cnt;
			nbytes = page_num_diff < PAGES_PER_BUFFER ?
...
int main(int argc, char *args[])
{
	...
	for (int i = 0; i < NUM_FILES; ++i) {
		...
		/* Use size of buffer for size of batch */
		for (int j = 0; j < PAGES_PER_FILE; j += (2 * PAGES_PER_BUFFER)) {
			set_pages(&ud, fd, i, j, j + PAGES_PER_BUFFER - 1, &nr_page,
					   page_nums);
		}
	}
