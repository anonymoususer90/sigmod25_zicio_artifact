 ...
 * Return -1, error
 */
static int do_data_ingestion(struct zicio *zicio, int process_id, int nr_page,
							 unsigned long long *page_nums)
{
	...
	printf("[PROCESS %d COMPLETE] cnt: %d, nr_page: %d\n", process_id, cnt, nr_page);
...
void proc_func()
{
	...
	zicio.zicio_flag = 0;
	zicio.read_page_size = PAGE_SIZE; // 8 KiB
	...
	for (int i = 0; i < NUM_FILES; ++i) {
		...
		set_pages(&zicio, fd, i, 0, 3, &nr_page, page_nums);			// +4
		set_pages(&zicio, fd, i, 3, 7, &nr_page, page_nums);			// +5
		set_pages(&zicio, fd, i, 8, 11, &nr_page, page_nums);			// +4
		set_pages(&zicio, fd, i, 16, 19, &nr_page, page_nums);			// +4
		set_pages(&zicio, fd, i, 256, 511, &nr_page, page_nums);		// +256
		set_pages(&zicio, fd, i, 1024, 2047, &nr_page, page_nums);		// +1024
		set_pages(&zicio, fd, i, 262140, 262143, &nr_page, page_nums);	// +4
	}
	...
	ret = do_data_ingestion(&zicio, proc_id, nr_page, page_nums);
...
/* 
 * Batched data ingestion test - read 16 GiB from one 8 files with 16 threads
 */
