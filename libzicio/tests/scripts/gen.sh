...
then
	...
	# Make data files
	if [ ! -e ${TEST_PATH}/data.0 ]; then
		echo "Make data files"
		../gen 2
		mv data ${TEST_PATH}/data.0
	fi

	for VAR in {1..7};
	do
		if [ ! -e ${TEST_PATH}/data.${VAR} ]; then
			cp ${TEST_PATH}/data.0 ${TEST_PATH}/data.${VAR} &
		fi
	done
	# Make not aligned data
	if [ ! -e ${TEST_PATH}/not_aligned_data.0 ]; then
		echo "Make not aligned data file"
		../gen_kb 16800
		mv data ${TEST_PATH}/not_aligned_data.0
	fi
else
	echo "Make data files"
