...
/* 
 * Regression test with multi thread - read 2 GiB (i.e. 2 files) with multi threads
 */
int main(int argc, char *args[])
{
	...
	/* Set the # of threads be executed */
  	num_threads = CPU_COUNT(&cpuset) * 2;

	threads = (pthread_t *) malloc(sizeof(pthread_t) * num_threads);
	thread_args = (int *) malloc(sizeof(int) * num_threads);
	...
	memset(thread_args, 0, sizeof(int) * num_threads);
