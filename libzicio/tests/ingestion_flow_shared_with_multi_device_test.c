...
int main(int argc, char *args[])
{
	...
	for (int i = 0 ; i < num_data_path ; i++) {
        if (access(args[i + 3], 0)) {
			printf("data_path[%s] is invalid\n", args[i + 3]);
			free(data_path);
			return -1;
		}
		data_path[i] = args[i + 3];
	}
	...
	for (int i = 0 ; i < NUM_FILES ; i++) {
		...
		sprintf(path, "%s/data.%d", data_path[i % num_data_path], i);
