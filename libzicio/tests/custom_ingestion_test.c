...
static int do_data_ingestion(struct zicio *zicio)
{
	...
	/* Print read result */
	//printf("[DATA INGESTION COMPLETE] v1: %lu, v2: %lu, etc: %lu\n", 
	//	v1, v2, etc);
	 ...
	 */
	if (etc > 0 || v1 != v2 || 
	  	(page_cnt * zicio->read_page_size) != __ZICIO_LIB_CHUNK_SIZE * 1024)
		return -1;
	else
		return 0;
}
...
int main(int argc, char *args[])
{
	...
	for (int i = 0; i < 13; ++i) {
		...
		zicio.nr_local_fd = nr_fd;
		zicio.nr_shareable_fd = 0;
		zicio.zicio_flag = 0;
		zicio.read_page_size = read_page_sizes[i];
