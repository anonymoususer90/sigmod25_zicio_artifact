...
static int pread_data_get_page(struct pread_data *pd) {
	...
	if (idx == 0) {
		...
		offset = ((unsigned long long)(pd->cur_ingested_page_cnt) -
				  accumulated_nr_pages[pd->cur_fd_idx]) * 2 * PAGE_SIZE;
		...
		pread(pd->fds[pd->cur_fd_idx], pd->buffer, nbytes, offset);
...
int main(int argc, char *args[])
{
	...
	for (int i = 0; i < NUM_FILES; ++i) {
		...
		for (unsigned long long j = 0; j < PAGES_PER_FILE;
			 j += (2 * PAGES_PER_BUFFER)) {
			set_pages(&pd, fd, i, j, j + PAGES_PER_BUFFER - 1, &nr_page,
					   page_nums);
		}
	}
