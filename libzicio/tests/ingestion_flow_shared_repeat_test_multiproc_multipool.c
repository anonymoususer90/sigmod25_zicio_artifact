...
struct zicio zicio_shared_pool[NUM_POOL];
int proc_id;
...
void proc_func(int *fds, int nr_fd)
{
	struct zicio zicio;
	int ret = 0;
	int shared_idx = proc_id % NUM_POOL;

	for (int j = 0 ; j < num_repeat ; j++) {
		assert(zicio_shared_pool[shared_idx].zicio_shared_pool_key);

		zicio_init(&zicio);
		do_init_shared_zicio_args(&zicio, fds, nr_fd, zicio_shared_pool[shared_idx].zicio_shared_pool_key);
...
int main(int argc, char *args[])
{
	...
	for (int i = 0 ; i < NUM_POOL ; i++) {
		...
		if (zicio_destroy_pool(zicio_shared_pool[i].zicio_shared_pool_key) < 0) {
