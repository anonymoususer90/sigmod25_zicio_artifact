...
def get_query_parameters(query_no, scale_factor):      
    if query_no == 1:
        ...
        return [size, "%" + type, region, region]
    elif query_no == 3:
        ...
        return [nation, fraction, nation]
    elif query_no == 12:
        mode_list = random.sample(modes, 2)
        mode_1 = mode_list[0]
        mode_2 = mode_list[1]
        date = random_date_with_first_month_and_first_day(datetime(1993, 1, 1), datetime(1997, 12, 31))
        return [mode_1, mode_2, date, date]
    elif query_no == 13:
        ...
        stream_id = threading.get_ident() % 1000
        date = random_date_with_first_day(datetime(1993, 1, 1), datetime(1997, 10, 31))
        return [stream_id, date, date, stream_id, stream_id, stream_id]
    elif query_no == 16:
        ...
        return [brand_1, quantity_1, quantity_1, \
                brand_2, quantity_2, quantity_2, \
                brand_3, quantity_3, quantity_3]
    elif query_no == 20:
        color = random.choice(P_NAME)
        date = random_date_with_first_month_and_first_day(datetime(1993, 1, 1), datetime(1997, 12, 31))
        nation = random.choice(nations)[N_NAME]
        return [color + "%", date, date, nation]
    elif query_no == 21:
        nation = random.choice(nations)[N_NAME]
        return [nation]
    elif query_no == 22:
