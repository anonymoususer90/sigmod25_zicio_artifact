...
#define ZICIO_GLOBALTABLESPACE_OID 1664
...
/* session flag to enable zicio channel */
extern bool enable_zicio;
/* session flag to enable zicio channel */
extern bool enable_zicio_shared_pool;
extern int current_query_number;

/*
 * pg_enable_zicio
 * Enable sequential scan with zicio
 */
Datum
pg_enable_zicio(PG_FUNCTION_ARGS)
{
	enable_zicio = true;
...
/*
 * pg_disable_zicio
 * Disable sequential scan with zicio
 */
Datum
pg_disable_zicio(PG_FUNCTION_ARGS)
{
	enable_zicio = false;
...
/*
 * pg_enable_zicio_shared_pool
 * Enable sequential scan with zicio shared pool.
 */
Datum
pg_enable_zicio_shared_pool(PG_FUNCTION_ARGS)
{
	int queryNumber = PG_GETARG_INT32(0);

	/* Currently, we don't allow to use zicio shared pool without turning
	 * on using zicio. */
	if (!enable_zicio) {
		return false;
	}
	enable_zicio_shared_pool = true;
	current_query_number = queryNumber;

	return enable_zicio_shared_pool;
}


/*
 * pg_disable_zicio_shared_pool
 * Disable zicio scan with shared pool.
 */
Datum
pg_disable_zicio_shared_pool(PG_FUNCTION_ARGS)
{
	enable_zicio_shared_pool = false;
...
/*
 * pg_zicio_get_querynumber
 */
Datum
pg_zicio_get_querynumber(PG_FUNCTION_ARGS)
{
	return current_query_number;
}

/*
 * pg_create_zicio_shared_pool
 *
 * Create shared pool with the name of relation.
 * PG_FUNCTION_ARGS: the name of relation.
 */
Datum
pg_create_zicio_shared_pool(PG_FUNCTION_ARGS)
{
	Oid relid = PG_GETARG_OID(0);
	int queryNumber = PG_GETARG_INT32(1);
	Relation relation;
	int fds[MAX_NUM_ZICIO_FD];
	int fdNums;
	SMgrRelation smgr;
	struct zicio zic;

	/* Check if target already exits. */
	ZicioSharedpoolLockAcquire(LW_SHARED);
	if (FindZicioSharedpool(relid, queryNumber))
	{
		elog(WARNING, "shared pool already exists.");
		ZicioSharedpoolLockRelease();
		return false;
	}
	ZicioSharedpoolLockRelease();

	/* Get relation from relation oid. */
	relation = RelationIdGetRelation(relid);
	smgr = RelationGetSmgr(relation);

	/* Get fds */
	zicio_smgrfds(smgr, MAIN_FORKNUM, fds, &fdNums);
	RelationClose(relation);

	/* Acquire lock. */
	ZicioSharedpoolLockAcquire(LW_EXCLUSIVE);

	/* Initialize zicio structure. */
	zicio_init(&zic);
	
	/* Init zicio for sharing */
	zic.shareable_fds = fds;
	zic.nr_shareable_fd = fdNums;
	zic.zicio_flag = 0;
	zic.read_page_size = BLCKSZ;
	zic.zicio_shared_pool_key = 0;

	zicio_create_pool(&zic);

	/* Insert created sharedpool to array. */
	InsertZicioSharedpool(relid, queryNumber, zic.zicio_shared_pool_key, 
																fds, fdNums);
...
/*
 * pg_destroy_zicio_shared_pool
 *
 * Destroy shared pool.
 * PG_FUNCTION_ARGS: relation oid, query number
 */
Datum
pg_destroy_zicio_shared_pool(PG_FUNCTION_ARGS)
{
	Oid relid = PG_GETARG_OID(0);
	int queryNumber = PG_GETARG_INT32(1);
	...
	{
		ZicioSharedpoolLockRelease();
		elog(LOG, "we cannot find sharedpool");
		return false;
	}

	if (zicio_destroy_pool(sharedpool->zicioSharedpoolKey) < 0)
	{
		ZicioSharedpoolLockRelease();
		elog(LOG, "[ERROR] zicio_detroy_pool error");
		return false;
	}

	/*
	 * After destroying the pool, close files for this shared pool.
	 */
	for (int i = 0 ; i < sharedpool->fdNums ; ++i)
		close(sharedpool->fds[i]);

	/* Delete shared pool from array. */
	DeleteZicioSharedpool(relid, queryNumber);
