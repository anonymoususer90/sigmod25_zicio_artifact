...
unsigned long long seq_buffer_wait_time;
unsigned long long physical_io_time_with_buf;
unsigned long long physical_io_time_without_buf;
unsigned long long zicio_wait_time;
bool physical_io_with_buf;
char *table_stat_msg = NULL;
#endif
			...
			case 'Q':			/* simple query */
				{
					...
					if (query_string[3] == 'Q')
					{
						...
						seq_buffer_wait_time = 0ULL;
						physical_io_time_with_buf = 0ULL;
						physical_io_time_without_buf = 0ULL;
						zicio_wait_time = 0ULL;

                        physical_io_with_buf = false;
						...
						/* Allocate buffer to write query message */
						file_path = 
							(char *) palloc(sizeof(char) * MESSAGE_SIZE);
						output_msg = 
							(char *) palloc(sizeof(char) * OUTPUT_MESSAGE_SIZE);
						/*
						if (table_stat_msg == NULL)
						{
							table_stat_msg = 
								(char *) palloc(sizeof(char) * OUTPUT_MESSAGE_SIZE);
						}*/

						memset(file_path, 0, sizeof(char) * MESSAGE_SIZE);
						memset(output_msg, 0, sizeof(char) * OUTPUT_MESSAGE_SIZE);
						if (table_stat_msg != NULL)
							memset(table_stat_msg, 0, sizeof(char) * OUTPUT_MESSAGE_SIZE);

						/* Set file path for writing results */
						sprintf(file_path, "results/%d.out", getpid());

						/* Open file for query stat */
					...
					if (query_string[3] == 'Q')
					{
						zicio_pread_breakdown_result result;
						...
						//result = zicio_u_pread_breakdown_end();
 					
						/* Write elapsed time of query */
						sprintf(output_msg, 
							"[QUERY RESULT] %c%c%c, pid %d, execution_time(s) %.6lf, buffer_wait_time(s) %.6lf, seq_buffer_wait_time(s) %.6lf, zicio_wait_time(s) %.6lf, physical_io_time_with_buf(s) %.6lf, physical_io_time_without_buf(s) %.6lf, mode_switch(s) %.6lf, data_copy(s) %.6lf, storage_stack(s) %.6lf, physical_io(s) %.6lf\n",
							query_string[3],
							query_string[4],
							query_string[5],
							getpid(),
							time_span_ns / 1e9,
							buffer_wait_time / 1e9,
							seq_buffer_wait_time / 1e9,
							zicio_wait_time / 1e9,
							physical_io_time_with_buf / 1e9,
							physical_io_time_without_buf / 1e9,
							result.mode_switch_latency / 1e9,
							result.data_copy_latency / 1e9,
							result.storage_stacks_latency / 1e9,
							result.physical_io_latency / 1e9);
					
						/* Concatenate output messsage and table stat message */
						if (table_stat_msg)
							strcat(output_msg, table_stat_msg);
