...
#ifdef ZICIO
	void 		(*smgr_zicio_read) (SMgrRelation reln, ForkNumber forknum, 
								BlockNumber blocknum, char *buffer, uint64_t *read_nbytes);
	void		(*smgr_zicio_get_fds) (SMgrRelation reln, ForkNumber forknum, 
								int *fds, int *nr_fd);
#endif
...
static const f_smgr smgrsw[] = {
	/* magnetic disk */
	{
		...
		.smgr_read = mdread,
#ifdef ZICIO
		.smgr_zicio_read = zicio_mdread,
		.smgr_zicio_get_fds = zicio_get_fds,
#endif
		.smgr_write = mdwrite,
...
#ifdef ZICIO
/*
 *	zicio_smgrread() -- read a particular block from a relation into the 
 *							supplied  buffer.
 *
 *		This routine is called from the buffer manager in order to
 *		instantiate pages in the shared buffer cache.  All storage managers
 ...
 */
void
zicio_smgrread(SMgrRelation reln, ForkNumber forknum, 
					BlockNumber blocknum, char *buffer, uint64_t *read_nbytes)
{
	smgrsw[reln->smgr_which].smgr_zicio_read(reln, forknum, blocknum, 
														buffer, read_nbytes);
}

/*
 *	zicio_smgrfds() -- Get fds in the supplied relation.
 */
void
zicio_smgrfds(SMgrRelation reln, ForkNumber forknum, int *fds, int *nr_fd)
{
	smgrsw[reln->smgr_which].smgr_zicio_get_fds(reln, forknum, fds, nr_fd);
}
#endif
