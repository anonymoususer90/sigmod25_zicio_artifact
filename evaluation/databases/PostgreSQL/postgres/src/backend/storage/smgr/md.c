...
#ifdef ZICIO
/*
 *	zicio_mdread() -- Read the chunk from a relation.
 */
void
zicio_mdread(SMgrRelation reln, ForkNumber forknum, BlockNumber blocknum,
	   char *buffer, uint64_t *read_nbytes)
{
	off_t		seekpos;
	int			nbytes;
	MdfdVec    *v;
	...
	nbytes = FileRead(v->mdfd_vfd, buffer, ZICIO_CHUNK_SIZE, 
														seekpos, WAIT_EVENT_DATA_FILE_READ);
...
/*
 *	zicio_get_fds() -- Get and set fd array for santface
 */
void
zicio_get_fds(SMgrRelation reln, ForkNumber forknum, int *fds, int *nr_fd)
{
	BlockNumber nblocks;
	BlockNumber segno = 0;
	char		*path;
	...
	fd = open(path, O_RDWR | PG_BINARY | O_DIRECT);
	...
	for (;;)
	{
		...
		len = lseek(fds[idx], 0, SEEK_END);
		if (len < 0) {
			elog(ERROR, "lseek error: %d\n", fds[idx]);
			return;
		}

		Assert(idx < MAX_NUM_ZICIO_FD);
		idx += 1;

		/* note that this calculation will ignore any partial block at EOF */
		nblocks = (BlockNumber) (len / BLCKSZ);
		...
		fd = open(fullpath, O_RDWR | PG_BINARY | O_DIRECT); 
