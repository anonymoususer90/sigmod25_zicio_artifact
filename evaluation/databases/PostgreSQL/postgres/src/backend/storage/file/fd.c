...
extern unsigned long long physical_io_time_with_buf;
extern unsigned long long physical_io_time_without_buf;
extern bool physical_io_with_buf;
#endif
		 ...
		 uint32 wait_event_info)
{
	...
	unsigned long long time_span_ns;
#endif
...
retry:
...
#ifdef ZICIO_STAT
	returnCode = zicio_pread(vfdP->fd, buffer, amount, offset);
#else
	...
	if (physical_io_with_buf)
	{
		physical_io_time_with_buf += time_span_ns;
	}
	else
	{
		physical_io_time_without_buf += time_span_ns;
	}
#endif
	pgstat_report_wait_end();
