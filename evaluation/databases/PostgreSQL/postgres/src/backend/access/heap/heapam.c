...
#if defined(ZICIO) || defined(ZICIO_STAT)
#include "utils/rel.h"
...
extern uint32 lineitem_shared_pool_key;
#endif

#ifdef ZICIO_STAT
extern unsigned long long buffer_wait_time;
extern unsigned long long seq_buffer_wait_time;
extern unsigned long long zicio_wait_time;

extern char *table_stat_msg;
#endif
...
#ifdef ZICIO
/*
 * Flag to determine to enable zicio workload.
 */
bool enable_zicio = false;
bool enable_zicio_shared_pool = false;
int current_query_number = 0;
#endif
...
#ifdef ZICIO_STAT
	scan->buffer_wait_time = 0ULL;
	scan->zicio_wait_time = 0ULL;
#endif
...
#ifdef ZICIO
	/*
	 * Initialize scan descriptor for zicio channel.
	 */
	scan->zicio_init = 0;

	scan->zicio_chunk = NULL;
	scan->zicio_cdp = NULL;

	/* 
	 * We scan using zicio only for user level table and only use zicio 
	 * for tables larger than 2 MiB.
	 */
	if (!IsSystemRelation(scan->rs_base.rs_rd) &&
			scan->rs_base.rs_rd->rd_node.dbNode != 1 &&
			(scan->rs_base.rs_flags & SO_TYPE_SEQSCAN) &&
			scan->rs_base.rs_rd->is_systable == false &&
			scan->rs_nblocks > 256 && enable_zicio) {
		int tempFds[MAX_NUM_ZICIO_FD];
		int tempFdNums;
		SMgrRelation smgr;
		ZicioSharedpool sharedpool;
		int queryNumber;
		int relid;
	
		scan->zicio_init = 1;

		smgr = RelationGetSmgr(scan->rs_base.rs_rd);
		relid = RelationGetRelid(scan->rs_base.rs_rd);
		queryNumber = OidFunctionCall0(6126); /* pg_zicio_get_querynumber */

		sharedpool = FindZicioSharedpool(relid, queryNumber);

		/*
		 * Initialize structure.
		 */
		zicio_init(&scan->zicio);

		scan->zicio.zicio_flag = 0;
		scan->zicio.read_page_size = BLCKSZ;

		if (enable_zicio_shared_pool && sharedpool)
		{
			/* Get fds */
			zicio_smgrfds(smgr, MAIN_FORKNUM, scan->fds, &scan->nr_fd);
			Assert(scan->nr_fd > 0 && scan->nr_fd < MAX_NUM_ZICIO_FD);

			/* Init zicio for sharing */
			scan->zicio.local_fds = NULL;
			scan->zicio.shareable_fds = scan->fds;
			scan->zicio.nr_local_fd = 0;
			scan->zicio.nr_shareable_fd = scan->nr_fd;
			scan->zicio.zicio_shared_pool_key = sharedpool->zicioSharedpoolKey;

			fprintf(stderr, "[SHARING ATTACH] relname %s, "
					"read_page_size %d, nr_fd %d\n",
					RelationGetRelationName(scan->rs_base.rs_rd), BLCKSZ,
					scan->nr_fd);
		}
		else
		{
			/* Get fds */
			zicio_smgrfds(smgr, MAIN_FORKNUM, tempFds, &tempFdNums);
			Assert(tempFdNums > 0 && tempFdNums < MAX_NUM_ZICIO_FD);
			
			scan->nr_fd = tempFdNums;
			scan->zicio.local_fds = scan->fds;
			scan->zicio.shareable_fds = NULL;
			scan->zicio.nr_local_fd = 0;
			scan->zicio.nr_shareable_fd = 0;
			scan->zicio.zicio_shared_pool_key = 0;

			/* zicio_notify_ranges */
			for (int i = 0; i < tempFdNums; ++i)
			{
				int64_t fileSize = lseek(tempFds[i], 0, SEEK_END);
				int64_t blockNums = fileSize / BLCKSZ;

				if (blockNums > 0)
					zicio_notify_ranges(&scan->zicio, tempFds[i], 0, blockNums - 1);
			}

			fprintf(stderr, "[LOCAL ATTACH] relname %s, read_page_size %d, "
					"nr_fd %d\n", RelationGetRelationName(scan->rs_base.rs_rd),
					BLCKSZ, tempFdNums);
		}

		/* Open zicio */
		zicio_open(&scan->zicio);
		if (scan->zicio.open_status != ZICIO_OPEN_SUCCESS) {
			fprintf(stderr, "[ZICIO] zicio open fail\n");
			return;
		}
	}
#endif
...
#ifdef ZICIO
/*
 * zicio_heapgetpage - subroutine for heapgettup()
 *
 * Get page using zicio channel
 */
void
zicio_heapgetpage(TableScanDesc sscan, BlockNumber page)
{
	HeapScanDesc scan = (HeapScanDesc) sscan;
	Buffer		buffer;
	Snapshot	snapshot;
	...
	/* If it exists previous scan buffer, return error */
	Assert(!BufferIsValid(scan->rs_cbuf));

	/* We don't use shared buffer for zicio */
	...
	/* Get chunk if all pages inside chunk is exhausted */
	while (scan->zicio.get_status != ZICIO_GET_PAGE_SUCCESS) {
		zicio_get_page(&scan->zicio);

		CHECK_FOR_INTERRUPTS();
	}

	dp = (Page) scan->zicio.page_addr;
...
heapgetpage(TableScanDesc sscan, BlockNumber page)
{
	...
	unsigned long long time_span_ns = 0;
#endif
	...
	time_span_ns = 
		(end_time.tv_sec - start_time.tv_sec) * 1000000000UL +
			(end_time.tv_nsec - start_time.tv_nsec);

	/* add io time */
    seq_buffer_wait_time += time_span_ns;
	scan->buffer_wait_time += time_span_ns;
#endif
...
#ifdef ZICIO
			if (scan->zicio_init)
				zicio_heapgetpage((TableScanDesc) scan, page);
			else
				heapgetpage((TableScanDesc) scan, page);
#else
			heapgetpage((TableScanDesc) scan, page);
...
#ifdef ZICIO
			if (scan->zicio_init) {
				zicio_put_page(&scan->zicio);

				if (scan->zicio.put_status == ZICIO_PUT_PAGE_EOF) {
...
heap_endscan(TableScanDesc sscan)
{
	...
	if (scan->zicio_init) 
	{
		...
		wait_time_ns = zicio_get_wait_time(&scan->zicio); 

		/* Add io wait time when using zicio */
		scan->zicio_wait_time += wait_time_ns; 
		zicio_wait_time += wait_time_ns; 
	}
#endif

	if (table_stat_msg != NULL && scan->rs_base.rs_rd->rd_node.dbNode != 1) 
	{
		tmp = (char *) palloc(sizeof(char) * MESSAGE_SIZE);
		memset(tmp, 0, sizeof(char) * MESSAGE_SIZE);

		// Write table stat
		sprintf(tmp, 
			"\t[TABLE RESULT] seq scan, pid %d, xid %d, table %s, buffer_wait_time(ms) %.3lf, zicio_wait_time(ms) %.3lf\n", 
			getpid(),
			GetTopTransactionId(),
			RelationGetRelationName(scan->rs_base.rs_rd),
			scan->buffer_wait_time / 1e6,
			scan->zicio_wait_time / 1e6);
		strcat(table_stat_msg, tmp);
		pfree(tmp);
	}
#endif
	...
	if (scan->zicio_init) 
	{
		...
		/* Get zicio stat message and append it */
		if (table_stat_msg)
		{
			strcat(table_stat_msg, "\t\t[ZICIO RESULT] ");
			sf_stat_msg = zicio_get_stat_msg(&scan->zicio);
			strcat(table_stat_msg, sf_stat_msg);
			strcat(table_stat_msg, "\n");
			free(sf_stat_msg);
		}
		
		zicio_close(&scan->zicio);

		fprintf(stderr, "zicio close\n");

		Assert(scan->nr_fd < MAX_NUM_ZICIO_FD);
		for (i = 0; i < scan->nr_fd; ++i)
		{
			close(scan->fds[i]);
		}

		if (scan->zicio.close_status != ZICIO_CLOSE_SUCCESS) 
		{
			fprintf(stderr, "[ERROR] zicio close fail\n");
		}
	}
#endif
