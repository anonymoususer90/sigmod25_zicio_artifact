...
#define InvalidOffset ((uint64_t) -1)
#endif

#ifdef ZICIO_STAT
extern unsigned long long zicio_wait_time;
#endif
...
static ColumnBuffers * LoadColumnBuffers(Relation relation,
										 ...
										 ColumnChunkSkipNode *nextChunkSkipNodeArray,
										 ZICIODesc *desc);
#else
static ColumnBuffers * LoadColumnBuffers(Relation relation,
										 ColumnChunkSkipNode *chunkSkipNodeArray,
...
static Datum ColumnDefaultValue(TupleConstr *tupleConstraints,
								Form_pg_attribute attributeForm);
#ifdef ZICIO
//static int customPread(int fd, char *buf, int amount, off_t realOffset);
#endif 
...
#define EXISTS_CHUNK 0
#define VALUE_CHUNK 1
static void 
NotifyPagesInChunks(Relation relation, StripeMetadata *stripeMetadata,
	ColumnChunkSkipNode *chunkSkipNodeArray, long long *lastNotifiedBlockNum, 
						uint32 chunkCount, int whichChunk, ZICIODesc *desc)
{
	for (uint32 chunkIndex = 0; chunkIndex < chunkCount; chunkIndex++)
	{
		PhysicalAddr startPhysicalAddr;
		PhysicalAddr endPhysicalAddr;
		int startSegmentNum;
		BlockNumber startBlockNum;
		BlockNumber endBlockNum;
		BlockNumber endBlockNumInLastSeg;
		int endSegmentNum;
		ColumnChunkSkipNode *chunkSkipNode = 
									&chunkSkipNodeArray[chunkIndex];

		if(whichChunk == EXISTS_CHUNK)
		{
			uint64 startLogicalOffset = stripeMetadata->fileOffset + 
									chunkSkipNode->existsChunkOffset;

			Assert(chunkSkipNode->existsLength > 0);

			/* Get start and end physical address */
			startPhysicalAddr = ZICIOLogicalToPhysical(startLogicalOffset);
			endPhysicalAddr = ZICIOLogicalToPhysical(startLogicalOffset + 
											chunkSkipNode->existsLength - 1);
		}
		else
		{
			uint64 startLogicalOffset = stripeMetadata->fileOffset + 
									chunkSkipNode->valueChunkOffset;	

			Assert(chunkSkipNode->valueLength > 0);

			/* Get start and end physical address */
			startPhysicalAddr = ZICIOLogicalToPhysical(startLogicalOffset);
			endPhysicalAddr = ZICIOLogicalToPhysical(startLogicalOffset + 
											chunkSkipNode->valueLength - 1);
		}

		/* Get start segment number and end segment number */
		startBlockNum = startPhysicalAddr.blockno;
		endBlockNum = endPhysicalAddr.blockno;

		startSegmentNum = startBlockNum / ((BlockNumber) RELSEG_SIZE);
		endSegmentNum = endBlockNum / ((BlockNumber) RELSEG_SIZE);

		/* Get start block and end block for notify */
		endBlockNumInLastSeg = (endPhysicalAddr.blockno % 
									((BlockNumber) RELSEG_SIZE));

		/* 
		 * We notify pages to read in files with zicio_notify_pages. 
		 * If we should separate the notify for different segments, 
		 * we should call zicio_notify_pages for each segment.
		 */
		BlockNumber currentBlockNum = startBlockNum; 
		uint32 currentOffset = startPhysicalAddr.offset;

		for (uint64_t segmentNum = startSegmentNum; 
				segmentNum <= endSegmentNum; segmentNum++)
		{
			int fd;
			BlockNumber currentBlockNumInSeg;
			BlockNumber endBlockNumInSeg;
			uint64 currentLogicalOffset;

			/* Set PhysicalLocation of the current point */
			PhysicalAddr addr;
			addr.blockno = currentBlockNum;
			addr.offset = currentOffset;

			currentLogicalOffset = ZICIOPhysicalToLogical(addr);

			ZICIOGetPhysicalLocation(relation, currentLogicalOffset, NULL, &fd); 
			currentBlockNumInSeg = currentBlockNum % ((BlockNumber) RELSEG_SIZE);

			/* Get last block number in the segment */
			if (likely(segmentNum == endSegmentNum))
			{
				endBlockNumInSeg = endBlockNumInLastSeg;
			}
			else
			{
				uint64_t fileSize = lseek(fd, 0, SEEK_END);
				endBlockNumInSeg = ((BlockNumber) (fileSize / BLCKSZ)) - 1;

				Assert(endBlockNumInSeg < RELSEG_SIZE);
			}

			/* If we already notified the block, then we skip it */
			if (*lastNotifiedBlockNum == segmentNum * RELSEG_SIZE + currentBlockNumInSeg)
				currentBlockNumInSeg += 1;

			/* If there is no block to notify, then do nothing*/
			if (currentBlockNumInSeg <= endBlockNumInSeg)
			{
				/* Call zicio_notify_pages */
				zicio_notify_ranges(&desc->zicio, fd,
								currentBlockNumInSeg, endBlockNumInSeg);

				if (unlikely(desc->should_scan == false &&
							(endBlockNumInSeg - currentBlockNumInSeg + 1) > 0))
				{
					desc->should_scan = true;
				}

#ifdef ZICIO_DEBUG
				desc->notify_page_nums += (endBlockNumInSeg - currentBlockNumInSeg + 1);
#if 0
				if(whichChunk == EXISTS_CHUNK)
				{
					fprintf(stderr, "[ZIC_NOTIFY exists] segmentNum: %d chunkIndex: %d Offset: %ld length: %ld fd: %d start: %d end: %d, notify_nums: %d\n",
						segmentNum, chunkIndex, currentLogicalOffset,
						chunkSkipNode->existsLength, fd, \
						currentBlockNumInSeg, endBlockNumInSeg, desc->notify_page_nums);
				}
				else
				{
					fprintf(stderr, "[ZIC_NOTIFY value] segmentNum: %d chunkIndex: %d Offset: %ld length: %ld fd: %d start: %d end: %d, notify_nums: %d\n",
						segmentNum, chunkIndex, currentLogicalOffset,
						chunkSkipNode->valueLength, fd, \
						currentBlockNumInSeg, endBlockNumInSeg, desc->notify_page_nums);
				}
#endif
#endif
			...
			/* We must to set the last notified block not to notify again */
			*lastNotifiedBlockNum = (segmentNum * RELSEG_SIZE) + endBlockNumInSeg;

			/* Set PhysicalLogcation to the start location of the segment */
			currentBlockNum = *lastNotifiedBlockNum + 1;
			currentOffset = SizeOfPageHeaderData;
		}	
	}
}
...
ZICIOInit()
{
	...
	/* Allocate zicio structure */
	zicio = (struct zicio *) palloc0(sizeof(struct zicio));

	zicio_init(zicio);

	/* Set values */
	zicio->local_fds = (int *) palloc0(sizeof(int) * maxFdNums);
	zicio->read_page_size = BLCKSZ;
						  ...
						  int64 *chunkGroupsFiltered, Snapshot snapshot)
{
...
#ifdef ZICIO_DEBUG
	zicioDesc.notify_page_nums = 0;
	zicioDesc.put_page_nums = 0;
	zicioDesc.get_page_nums = 0;
#endif
	zicioDesc.should_scan = false;
	
	/* Initialize a structure for ZICIO */
	zicio_init(&zicioDesc.zicio);

	/* Set values */
	zicioDesc.zicio.local_fds = fds;
	zicioDesc.zicio.read_page_size = BLCKSZ;

	long long lastNotifiedBlockNum = -1;
	for (columnIndex = 0; columnIndex < stripeMetadata->columnCount; \
			columnIndex++)
	{
		if (projectedColumnMask[columnIndex])
		{
			...
			/* Notify exists chunks */
			NotifyPagesInChunks(relation, stripeMetadata, chunkSkipNodeArray,
					&lastNotifiedBlockNum, chunkCount, EXISTS_CHUNK, &zicioDesc);
			/* Notify value chunks */
			NotifyPagesInChunks(relation, stripeMetadata, chunkSkipNodeArray,
					&lastNotifiedBlockNum, chunkCount, VALUE_CHUNK, &zicioDesc);
		}
	}

#ifdef ZICIO_DEBUG
	if (zicioDesc.notify_page_nums > 0)
		fprintf(stderr, "scan with zicIO, rel: %s, notify_nums: %d\n",
			RelationGetRelationName(relation), zicioDesc.notify_page_nums);
#endif

	if (zicioDesc.should_scan)
	{
		/* ZICIO start */
		zicio_open(&zicioDesc.zicio);
	}
#endif
...
#ifdef ZICIO
			/* 
			 * We compare current column group and next column group to
			 * control get and put flow. If these two column cgroups are in the
			 * same block, we don't call zicio_put_page function for the next
			 * chunk group.
			 */
			ColumnChunkSkipNode *nextChunkSkipNode = NULL;
			uint64 nextColumnIndex = columnIndex + 1;
			while (nextColumnIndex < stripeMetadata->columnCount)
			{
				if (projectedColumnMask[nextColumnIndex])
				{
					nextChunkSkipNode = 
						selectedChunkSkipList->chunkSkipNodeArray[nextColumnIndex];
					break;
				}
	...
	if (zicioDesc.should_scan)
	{
		...
		wait_time_ns = zicio_get_wait_time(&zicioDesc.zicio);

		/* Add io wait time when using zicio */
		zicio_wait_time += wait_time_ns;
#endif

		/* ZICIO close */
		zicio_close(&zicioDesc.zicio);
	}

#ifdef ZICIO_DEBUG
	Assert(zicioDesc.notify_page_nums == zicioDesc.get_page_nums);
	Assert(zicioDesc.get_page_nums == zicioDesc.put_page_nums);
#endif
#endif
...
#endif
{
...
#ifdef ZICIO
	bool shouldPut;
	ColumnChunkSkipNode *nextChunkSkipNode;
	uint64 currentEndOffset;
	uint64 nextStartOffset;
	uint64 currentPhysicalEndOffset;
	uint64 nextPhysicalStartOffset;
	int currentFd, nextFd;
#endif
	...
	for (chunkIndex = 0; chunkIndex < chunkCount; chunkIndex++)
	{
		...
		/* 
		 * We compare current chunk and next chunk to control get and put
		 * flow. If these two chunks are in the same block, we don't call 
		 * zicio_put_page function for the next chunk.
		 */
		if (chunkIndex + 1 != chunkCount)
		{
			nextChunkSkipNode = &chunkSkipNodeArray[chunkIndex + 1];
			nextStartOffset = stripeOffset + nextChunkSkipNode->existsChunkOffset;
		}
		else
		{
		  	nextChunkSkipNode = &chunkSkipNodeArray[0];
			nextStartOffset = stripeOffset + nextChunkSkipNode->valueChunkOffset;
		}
		
		currentEndOffset = existsOffset + chunkSkipNode->existsLength - 1;

		/* Change logical offset to physical offset in file. */
		ZICIOGetPhysicalLocation(relation, currentEndOffset,
									&currentPhysicalEndOffset, &currentFd);
		ZICIOGetPhysicalLocation(relation, nextStartOffset,
									&nextPhysicalStartOffset, &nextFd);

		/* If they are in same file and same block, we must not to put. */
		if (currentFd == nextFd &&
			(currentPhysicalEndOffset / BLCKSZ == nextPhysicalStartOffset / BLCKSZ))
		{
			shouldPut = false;
		}
#endif
		...
		ColumnarStorageRead(relation, existsOffset, rawExistsBuffer->data,
							chunkSkipNode->existsLength, shouldPut, desc);
#else
		ColumnarStorageRead(relation, existsOffset, rawExistsBuffer->data,
	...
	for (chunkIndex = 0; chunkIndex < chunkCount; chunkIndex++)
	{
...
#ifdef ZICIO
		/* 
		 * We compare current value and next value to control get and put
		 * flow. If these two values are in the same block, we don't call 
		 * zicio_put_page function for the next value.
		 */
		shouldPut = true;

		currentEndOffset = valueOffset + chunkSkipNode->valueLength - 1;
		...
		else if (nextChunkSkipNodeArray != NULL)
		{
		  	nextChunkSkipNode = &nextChunkSkipNodeArray[0];
			nextStartOffset = stripeOffset + nextChunkSkipNode->existsChunkOffset;
		}
		...
		if (chunkIndex + 1 != chunkCount || nextChunkSkipNodeArray != NULL)
		{
			/* If they are in same file and same block, we must not to put. */
			if (currentFd == nextFd &&
				(currentPhysicalEndOffset / BLCKSZ == nextPhysicalStartOffset / BLCKSZ))
		...
		ColumnarStorageRead(relation, valueOffset, rawValueBuffer->data,
							chunkSkipNode->valueLength, shouldPut, desc);
#else
		ColumnarStorageRead(relation, valueOffset, rawValueBuffer->data,
