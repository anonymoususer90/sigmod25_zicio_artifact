...
static char *ZICIOptr = NULL;
#endif

#ifdef ZICIO_STAT
extern uint64_t buffer_wait_time;
extern uint64_t seq_buffer_wait_time;
extern uint64_t zicio_wait_time;
...
#ifdef ZICIO
PhysicalAddr
ZICIOLogicalToPhysical(uint64 logicalOffset)
{
	return LogicalToPhysical(logicalOffset);
}

uint64
ZICIOPhysicalToLogical(PhysicalAddr addr)
{
	uint64 logicalOffset;

	logicalOffset = (addr.blockno * COLUMNAR_BYTES_PER_PAGE) +
					(addr.offset - SizeOfPageHeaderData);
...
static void ReadFromBlock(Relation rel, BlockNumber blockno, uint32 offset,
						  char *buf, uint32 len, bool force, bool inMeta,
						  bool shouldPut, ZICIODesc *desc);
#else
static void ReadFromBlock(Relation rel, BlockNumber blockno, uint32 offset,
...
#ifdef ZICIO
/*
void ZICIOGetFd(Relation rel, uint64 physicalOffset, int *fd);
{
}
*/
...
void
ZICIOGetPhysicalLocation(Relation rel, uint64 logicalOffset,
							uint64 *physicalOffset, int *fd)
{
	...
	{
		elog(ERROR,
			"attempted zicio getOffset on relation %d from invalid logical offset: "
			UINT64_FORMAT,
			rel->rd_id, logicalOffset);
	}

	addr = LogicalToPhysical(logicalOffset);

	seekpos = (off_t) BLCKSZ * ((uint64_t) addr.blockno);
	offsetInBlock = addr.offset;

	/* set fd and physicalOffset */
	if (fd != NULL)
		*fd = smgrgetfd(RelationGetSmgr(rel), MAIN_FORKNUM, addr.blockno);
	if (physicalOffset != NULL)
		*physicalOffset = (uint64_t) (seekpos + offsetInBlock);
}
#endif
...
#ifdef ZICIO
void
ColumnarStorageRead(Relation rel, uint64 logicalOffset, char *data,
							uint32 amount, bool shouldPut, ZICIODesc *desc)
#else
void
...
#ifdef ZICIO
		if (read + to_read == amount)
		{
			/* Case 1: end of request */
			ReadFromBlock(rel, addr.blockno, addr.offset, data + read, to_read,
					  					false, false, shouldPut, desc);
		}
		else
		{
			/* Case 2: not end of request,  */
			ReadFromBlock(rel, addr.blockno, addr.offset, data + read, to_read,
...
#ifdef ZICIO
static void
ZICIODirectRead(Relation rel, BlockNumber blocknum, char *buffer)
{
	mdread(RelationGetSmgr(rel), MAIN_FORKNUM, blocknum, buffer);
}
#endif
...
ReadFromBlock(Relation rel, BlockNumber blockno, uint32 offset, char *buf,
				uint32 len, bool force, bool inMeta, bool shouldPut,
				ZICIODesc *desc)
#else
static void
ReadFromBlock(Relation rel, BlockNumber blockno, uint32 offset, char *buf,
...
#endif
{
	...
	else
	{
		...
		if (ZICIOptr == NULL)
		{
			/* Call zicio_get_page() and set ZICIOptr */
...
#ifdef ZICIO_DEBUG
			desc->get_page_nums += 1;
			Assert(desc->get_page_nums <= desc->notify_page_nums);
			//fprintf(stderr, "zicio_get_page blkno: %d, offset: %u, len: %u, get_nums: %d\n", 
			//							blockno, offset, len, desc->get_page_nums);
#endif

			/* Get page from zicio channel */
			page = zicio->page_addr;
			ZICIOptr = (char*) page;
		}
		else
		{
	...
	if (shouldPut)
	{
		...
		/* Call zicio_put_page and initialize ZICIOptr */
		zicio_put_page(zicio);
		ZICIOptr = NULL;

#ifdef ZICIO_DEBUG
		desc->put_page_nums += 1;
		Assert(desc->get_page_nums == desc->put_page_nums);
		Assert(desc->put_page_nums <= desc->notify_page_nums);
		//fprintf(stderr, "zicio_put_page blkno: %d, offset: %u, len: %u, put_nums: %d\n", 
		//								blockno, offset, len, desc->put_page_nums);
#endif
	}
