...
bool Sql_cmd_call::execute_inner(THD *thd) {
  ...
  zicio_pread_breakdown_result pread_result = { 0, };
...
#ifdef __ZICIO_STAT
  stopwatch.enable_zicio_stat = thd->variables.enable_zicio_stat;

  /* Copy the original query string */
  if (stopwatch.enable_zicio_stat)
    strcpy(stopwatch.query, proc_name->m_name.str);
#endif /* __ZICIO_STAT */
  ...
  */
  ...
  if (stopwatch.enable_zicio_stat) {
    /* init values */
    stopwatch.counter = 0;
    stopwatch.global_elapsed_io_time = 0;
    stopwatch.scan_elapsed_io_time = 0;
    stopwatch.index_elapsed_io_time = 0;
    stopwatch.lookup_elapsed_io_time = 0;

#ifdef PREAD_BREAKDOWN
	  zicio_u_pread_breakdown_start(false);
#endif /* PREAD_BREAKDOWN */
  ...
  if (stopwatch.enable_zicio_stat) {
    ...
    /* Get execution time */
    ulonglong execution_time = ((end.tv_sec * 1000000000 + end.tv_nsec) -
                              (begin.tv_sec * 1000000000 + begin.tv_nsec));
    
    /* Print log */
    fprintf(stderr, "[stat] [IO] query %s, execution_time(ns) %llu, global_elapsed_IO_time(ns) %llu, scan_elapsed_IO_time(ns): %llu, index_elapsed_IO_time(ns): %llu, lookup_elapsed_IO_time(ns): %llu\n",
            stopwatch.query, execution_time, stopwatch.global_elapsed_io_time, stopwatch.scan_elapsed_io_time, stopwatch.index_elapsed_io_time, stopwatch.lookup_elapsed_io_time);
#ifdef PREAD_BREAKDOWN
    fprintf(stderr, "[stat] [READ] query %s, mode_switch(ns, L2) %llu, data_copy(ns, L3) %llu, storage_stacks(ns, L4): %llu, physical_io(ns, L5): %llu\n",
            stopwatch.query, pread_result.mode_switch_latency, pread_result.data_copy_latency, pread_result.storage_stacks_latency, pread_result.physical_io_latency);
#endif /* PREAD_BREAKDOWN */
  }
#endif /* __ZICIO_STAT */
