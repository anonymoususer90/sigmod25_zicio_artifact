...
/**
  @brief
    This function create zicio shared pool for relation

  @param str   A String object that represent a relation

    Syntax:
      String zicio_create_shared_pool(table_name)

  @return returns a pointer to the string containing column options.
 */
String *Item_func_zicio_create_shared_pool::val_str(String *) {
  String dat_name;
  String rel_name;
  String pool_num;
  String *dat_name_ptr;
  String *rel_name_ptr;
  String *pool_num_ptr;
  char dat_path[FN_REFLEN];
  char rel_path[FN_REFLEN];
  const char *rel_suffix=".MYD";
  char pool_key[FN_REFLEN];

  /* Get database name */
  if (!(dat_name_ptr = args[0]->val_str(&dat_name)) ||
      !(current_thd->security_context()->check_access(FILE_ACL))) {
    assert(is_nullable());
    return error_str();
  }
  ...
  /* Get relation name */
  if (!(rel_name_ptr = args[1]->val_str(&rel_name)) ||
      !(current_thd->security_context()->check_access(FILE_ACL))) {
    assert(is_nullable());
    return error_str();
  }

  /* Get shared pool number */
  if (!(pool_num_ptr = args[2]->val_str(&pool_num)) ||
      !(current_thd->security_context()->check_access(FILE_ACL))) {
    assert(is_nullable());
    return error_str();
  }
  ...
  /* Check a pool number */
  if (std::atoi(pool_num_ptr->c_ptr_safe()) == INVALID_POOL_NUMBER)
    return error_str();

  /* Get hash key of shared pool */
  std::sprintf(pool_key, "%s%s", rel_name_ptr->c_ptr_safe(),
                                 pool_num_ptr->c_ptr_safe());

  /* Check a shared pool was created before */
  if (zicio_find_shared_pool(pool_key) != UINT32_MAX) {
    return error_str();
  }

  (void)fn_format(dat_path, dat_name_ptr->c_ptr_safe(), mysql_real_data_home, "",
                  MY_RELATIVE_PATH | MY_UNPACK_FILENAME);

  /* Read only allowed from within dir specified by secure_file_priv */
  if (!is_secure_file_path(dat_path)) {
    assert(is_nullable());
    return error_str();
  }

  (void)fn_format(rel_path, rel_name_ptr->c_ptr_safe(), dat_path, "",
                  MY_RELATIVE_PATH | MY_UNPACK_FILENAME);
  std::strcat(rel_path, rel_suffix);

  /* Read only allowed from within dir specified by secure_file_priv */
  if (!is_secure_file_path(rel_path)) {
    assert(is_nullable());
    return error_str();
  }

  /* Create zicio shared pool */
  if (!zicio_insert_shared_pool(pool_key, rel_path)) {
    return error_str();
  }

  tmp_value.length(0);
  tmp_value.append("Success", strlen("Success"));
  null_value = false;
...
/**
  @brief
    This function destroy zicio shared pool for relation

  @param str   A String object that represent a relation

    Syntax:
      String zicio_destroy_shared_pool(table_name)

  @return returns a pointer to the string containing column options.
 */
String *Item_func_zicio_destroy_shared_pool::val_str(String *) {
  String dat_name;
  String rel_name;
  String pool_num;
  String *dat_name_ptr;
  ...
  /* Delete zicio shared pool */
  if (!zicio_delete_shared_pool(pool_key)) {
    return error_str();
  }
