...
static Sys_var_bool Sys_enable_zicio(
    "enable_zicio",
    "Force sequential scan through zicio channel",
    SESSION_ONLY(enable_zicio), CMD_LINE(OPT_ARG), DEFAULT(false));

static Sys_var_uint Sys_zicio_shared_pool_num(
    "zicio_shared_pool_num",
    "Force sequential scan through zicio channel if the pool number is valid",
    SESSION_ONLY(zicio_shared_pool_num), CMD_LINE(OPT_ARG),
    VALID_RANGE(0, ~0), DEFAULT(INVALID_POOL_NUMBER), BLOCK_SIZE(1));
#endif /* __ZICIO_ACTIVATE */

#ifdef __ZICIO_STAT
static Sys_var_bool Sys_enable_zicio_stat(
    "enable_zicio_stat",
    "Enable zicio stat",
    SESSION_ONLY(enable_zicio_stat), CMD_LINE(OPT_ARG), DEFAULT(false));
#endif /* __ZICIO_STAT */
