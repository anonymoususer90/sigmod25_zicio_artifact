...
#ifdef __ZICIO_STAT
thread_local stopwatch_t stopwatch;

void stopwatch_set_begin() {
  /* Set timestamp */
  clock_gettime(CLOCK_MONOTONIC, &stopwatch.begin);
}

void stopwatch_set_end() {
  /* Set timestamp */
  clock_gettime(CLOCK_MONOTONIC, &stopwatch.end);

  /* Add elapsed time */
  ulonglong elapsed_io_time =
    ((stopwatch.end.tv_sec * 1000000000 + stopwatch.end.tv_nsec) -
   (stopwatch.begin.tv_sec * 1000000000 + stopwatch.begin.tv_nsec));
  stopwatch.once_elapsed_io_time += elapsed_io_time;
  stopwatch.global_elapsed_io_time += elapsed_io_time;
}
#endif /* __ZICIO_STAT */
