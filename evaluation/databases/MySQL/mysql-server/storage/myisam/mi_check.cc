...
int mi_repair_by_sort(MI_CHECK *param, MI_INFO *info, const char *name,
  ...
  if (!(sort_info.key_block =
      ...
      init_io_cache(&param->read_cache, info->dfile,
                    (uint)param->read_buffer_length, READ_CACHE,
                    share->pack.header_length + 8, true, MYF(MY_WME)) ||
      (!rep_quick &&
...
#ifdef __ZICIO_PAGINATION
    /* Init last page and current page */
    mi_max_offset_init(info);
    sort_param.max_pos = sort_param.pos = share->pack.header_length + 8;
#else /* !__ZICIO_PAGINATION */
  ...
  switch (share->data_file_type) {
    case STATIC_RECORD:
      for (;;) {
        ...
        /* Align in page units (2 MiB) */
        if (__get_offset(sort_param->pos) == info->cur_max_offset) {
          /* Set new max offset and align */
          if (likely(!mi_set_new_max_offset(info)))
            sort_param->pos = info->cur_page * __PAGE_SIZE + 8;
        }
      ...
      for (;;) {
        ...
        do {
          ...
          if (!found_record++) {
            ...
            /* Align in page units (2 MiB) */
            if (__get_offset(sort_param->pos) == info->cur_max_offset) {
              /* Checks whether this page is last */
              if (likely(!mi_set_new_max_offset(info)))
                sort_param->pos = info->cur_page * __PAGE_SIZE + 8;
            }
#endif /* __ZICIO_PAGINATION */
...
#ifdef __ZICIO_PAGINATION
        /* Align in page units (2 MiB) */
        if (__get_offset(pos) == info->cur_max_offset) {
          /* Checks whether this page is last */
          if (likely(!mi_set_new_max_offset(info)))
            pos = info->cur_page * __PAGE_SIZE + 8;
        }
#endif /* __ZICIO_PAGINATION */
