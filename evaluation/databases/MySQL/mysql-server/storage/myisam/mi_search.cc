...
my_off_t _mi_dpos(MI_INFO *info, uint nod_flag, const uchar *after_key) {
   ...
   *     0      4      8                              n
   *    -----------------------------------------------------------
   *     | page |maxoff| info->s->base.pack_reclength |    ...
   *    -----------------------------------------------------------
  ...
  if (!(info->s->options &
    ...
    /* Adjust offset for paginated data */
    uint rec_per_page = (__PAGE_SIZE - 8) / info->s->base.pack_reclength;
    uint page_num = pos / rec_per_page;
    uint offset = 8 + (pos % rec_per_page) * info->s->base.pack_reclength;
    return page_num * __PAGE_SIZE + offset;
  }
  else
    return pos;
#else
...
void _mi_dpointer(MI_INFO *info, uchar *buff, my_off_t pos) {
  if (!(info->s->options &
    ...
    /* If paginated, adjust the offset */
    uint rec_per_page = (__PAGE_SIZE - 8) / info->s->base.pack_reclength;
    pos = __get_page_number(pos) * rec_per_page +
         (__get_offset(pos) - 8) / info->s->base.pack_reclength;
#else
    pos /= info->s->base.pack_reclength;
#endif /* __ZICIO_PAGNIATION */
  }
