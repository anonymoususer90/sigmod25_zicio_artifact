...
int mi_create(const char *name, uint keys, MI_KEYDEF *keydefs, uint columns,
  ...
  /* Page number and max offset of first page */
  ulong start_page = 0;
...
#ifdef __ZICIO_PAGINATION
  /*
   * In pagination mode, the first page have to be written the number and last
   * offset also
   */
  share.state.state.data_file_length = 8;
#endif /* __ZICIO_PAGINATION */
...
#ifdef __ZICIO_PAGINATION
  /* Write page number and last offset of the first page */
  mysql_file_pwrite(dfile, (uchar*)(&start_page), 8, 0, MYF(MY_NABP));
#endif /* __ZICIO_PAGINATION */
