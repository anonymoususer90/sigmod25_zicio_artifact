...
int mi_open_datafile(MI_INFO *info, MYISAM_SHARE *share, const char *org_name,
  ...
  std::string dat_name;
  std::string rel_name;
  std::vector<std::string> tmp_name_vector;

  if (org_name) {
#ifdef __ZICIO_ACTIVATE
	/* Get the name of relation */
    std::istringstream name_stream(org_name);

    while (std::getline(name_stream, tmp_name, '/')) {
      tmp_name_vector.push_back(tmp_name);
    }
    rel_name = tmp_name_vector.back();
    std::strcpy(info->rel_name, rel_name.c_str());
#endif /* __ZICIO_ACTIVATE */
	...
	/* Get the name of relation */
    std::istringstream name_stream(data_name);
    ...
    rel_name = tmp_name_vector.back();
    std::strcpy(info->rel_name, rel_name.c_str());
    info->rel_name[strlen(info->rel_name) - 4] = 0;
  }
#endif /* __ZICIO_ACTIVATE */
