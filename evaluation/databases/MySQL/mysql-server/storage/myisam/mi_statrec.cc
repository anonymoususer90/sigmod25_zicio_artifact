...
int _mi_read_static_record(MI_INFO *info, my_off_t pos, uchar *record) {
  ...
  if (pos != HA_OFFSET_ERROR) {
    ...
    if (info->is_scanning) {
      ...
      my_off_t cur_pos = __get_offset(pos);
      memcpy(record, info->page + cur_pos, info->s->base.reclength);
      error = 0;
    } else {
...
int _mi_read_rnd_static_record(MI_INFO *info, uchar *buf, my_off_t filepos,
  ...
  if (unlikely(__get_offset(filepos) == info->cur_max_offset)) {
    ...
    if (likely((info->is_scanning = !info->is_all_read))) {
      /* Get new pate */
...
#ifdef __ZICIO_PAGINATION
  /* If read all of this file */
  if (unlikely(!info->is_scanning)) {
