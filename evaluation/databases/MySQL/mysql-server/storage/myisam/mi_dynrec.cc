...
extern my_off_t adjust_offset(MI_INFO *info, my_off_t pos, ulong size);
...
#ifdef __ZICIO_PAGINATION
int _mi_write_dynamic_record(MI_INFO *info,
                             const uchar *record __attribute__((unused))) {
  /* The length of the record is already determined in the mi_write() */
  return (write_dynamic_record(info, info->rec_buff, info->packed_length));
}
...
int _mi_write_blob_record(MI_INFO *info, const uchar *record) {
  uchar *rec_buff;
  memcpy(&rec_buff, &record, sizeof(uchar*));
  /* The length of the record is already determined in the mi_write(). */
  return write_dynamic_record(info, rec_buff, info->packed_length);
}
#else /* !__ZICIO_PAGINATION */
int _mi_write_blob_record(MI_INFO *info, const uchar *record) {
  uchar *rec_buff;
...
#ifdef __ZICIO_PAGINATION
    /* If next position is in the next page */
    if (__is_next_page(*filepos, tmp))
      *filepos = adjust_offset(info, *filepos, tmp);
#endif /* __ZICIO_PAGINATION */
  ...
  do {
...
#ifdef __ZICIO_PAGINATION 
    /* If read all of this page */
    if (unlikely(__get_offset(filepos) == info->cur_max_offset)) {
      /* Put this page and get new one */
      (*info->mi_put_page)(info);
      if (unlikely(info->is_all_read)) {
      ...
      /* Get new page */
      (*info->mi_get_page)(info);
 
      /* Adjust offset */
      filepos = info->cur_page * __PAGE_SIZE + 8;
    }
#else /* !__ZICIO_PAGINATION */
    ...
    if (info->opt_flag & READ_CACHE_USED) {
      ...
      if (_mi_read_cache(
        ...
        goto panic;
#ifdef IO_COUNT
      if (filepos < info->rec_cache.pos_in_file ||
      ...
      info->rec_cache.seek_not_done = true;
#ifdef __ZICIO_PAGINATION
      b_type = _mi_get_block_info_mem(&block_info, info->page, filepos);
#else /* !__ZICIO_PAGINATION */
    ...
    if (block_info.data_len) {
      if (info->opt_flag & READ_CACHE_USED) {
        ...
        if (info->opt_flag & WRITE_CACHE_USED &&
          ...
          goto err;
#ifdef __ZICIO_PAGINATION
        /* memcpy the record from page buffer instead of calling pread */
        my_off_t cur_pos = __get_offset(filepos);
        memcpy(to, info->page + cur_pos, block_info.data_len);
#else /* !__ZICIO_PAGINATION */
...
#ifdef __ZICIO_PAGINATION
uint _mi_get_block_info_mem(MI_BLOCK_INFO *info, uchar *buf, my_off_t filepos) {
  uint return_val = 0;
  ...
  if (buf != NULL) {
    /* memcpy the record from page buffer instead of calling pread */
    my_off_t cur_pos = __get_offset(filepos);
    memcpy(header, buf + cur_pos, sizeof(info->header));
  }
