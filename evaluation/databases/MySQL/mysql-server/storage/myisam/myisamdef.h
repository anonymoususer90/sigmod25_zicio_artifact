...
struct zicio_shared_pool_t {
  ...
  void zicio_destroy_shared_pool() {
    zicio_destroy_pool(zicio_shared_pool.zicio_shared_pool_key);
    mysql_file_close(fds[0], MYF(MY_WME));
  }
};

extern std::unordered_map<std::string, struct zicio_shared_pool_t*> zicio_shared_pool_map;
extern pthread_mutex_t zicio_shared_pool_lock;

#endif /* __ZICIO_ACTIVATE */

#define __PAGE_SHIFT (21)

/* Size of the page (default: 2 MiB) */
#define __PAGE_SIZE (1UL << __PAGE_SHIFT)

/* Mask the offset of page */
#define __PAGE_MASK (__PAGE_SIZE - 1)

/* Getting page number */
#define __get_page_number(filepos) \
  (((filepos) & (my_off_t)(~__PAGE_MASK)) >> __PAGE_SHIFT)

/* Getting offset */
#define __get_offset(filepos) \
  (((filepos) & (my_off_t)(__PAGE_MASK)))

/* If this record cannot to be inserted into current page */
#define __is_next_page(pos, size) \
 (__get_page_number(pos - 1 + size) != \
  __get_page_number(pos - 1))

/* Size of key buffer (for key-batching) */
#define __FILEPOS_BUFFER_SIZE (2097152UL)

/* Max count of batched key (for key-batching) */
#define __MAX_FILEPOS_COUNT ((uint)(__FILEPOS_BUFFER_SIZE / sizeof(my_off_t)))
...
#ifdef __ZICIO_PAGINATION
  int (*mi_put_page)(MI_INFO *);
  int (*mi_get_page)(MI_INFO *);
  int (*mi_pagination_free)(MI_INFO *);
#ifdef __ZICIO_ACTIVATE
  /* zicIO structure */
  struct zicio zicio;
  int zicio_channel_fds[MAX_NUM_ZICIO_FDS];
  bool enable_zicio;
  unsigned int zicio_shared_pool_num;
  char rel_name[FN_REFLEN];
#endif /* __ZICIO_ACTIVATE */
  /* Basic information for pagination */
  uint cur_page;            /* Page number currently being scanned */
  uint last_page;           /* Page number of the last page of file */
  uchar *page;              /* Current page buffer */
  my_off_t cur_max_offset;  /* Offset of end of this page */
  uint is_scanning;         /* Is this scan finished */
  uint is_all_read;         /* Are all pages read */
#endif /* __ZICIO_PAGINATION */
#ifdef IO_COUNT
  uint rows;
  uint io_count;
...
#ifdef __ZICIO_PAGINATION
extern int _mi_mem_read_cache(IO_CACHE *info, uchar *buff, my_off_t pos,
                              uint length, int re_read_if_possibly,
                              uchar *data_page);
#endif /* __ZICIO_PAGINATION */
...
#ifdef __ZICIO_PAGINATION
extern uint _mi_get_block_info_mem(MI_BLOCK_INFO *, uchar *, my_off_t);
#endif /* __ZICIO_PAGINATION */
