...
extern ulong _my_calc_total_blob_length(MI_INFO *info, const uchar *record);

#ifdef __ZICIO_PAGINATION
/*
 * adjust_offset
 *
 * @info - structure to adjust the data_file_length
 * @pos - offset to be adjusted
 * @size - size of new record
 *
 * Return : new adjusted offset
 *
 * Adjust the offset
 * In pagination mode, if the record cannot to be inserted into current page, it
 * has to be inserted into the next page.
 * This function adjust the offset to insert record into next page.
 */
my_off_t adjust_offset(MI_INFO *info, my_off_t pos, ulong size) { 
  /* Set the position of end of this file */
  my_off_t cur_offset = __get_offset(pos);
  my_off_t offset_position = ((pos - 1) & ~__PAGE_MASK) + 4; //start of the current page
  
  /* Write the offset of end of this page */
  info->s->file_write(info, (uchar*)(&cur_offset), 4, offset_position,
                      info->s->write_flag);

  /* Set filepos with new record */
  pos = (pos + size) & ~__PAGE_MASK;
  info->cur_page = __get_page_number(pos);

  /* Set page number */
  info->s->file_write(info, (uchar*)(&(info->cur_page)), 4, pos,
                      info->s->write_flag);
  
  /*
    4: page number
    4: last offet of this page
  */
  pos += 8;

  /* Adjust offset */
  info->state->data_file_length = pos;
...
#ifdef __ZICIO_PAGINATION
  /* For calculating size of BLOB record */
  /* Record buffer for getting size */
  uchar *rec_buff = NULL;
#endif /* __ZICIO_PAGINATION */
...
#ifdef __ZICIO_PAGINATION
  /* Variables for calculating size of record */
  ulong reclength, size, tmp;

  /* If this record is dynamic */
  if (share->options & HA_OPTION_PACK_RECORD) {
    /* Checking BLOB record */
    if (share->base.blobs) {
      /* Getting size of blob record */
      /* Referenced in _mi_write_blob_record() */
      ulong extra = (ALIGN_SIZE(MI_MAX_DYN_BLOCK_HEADER) + MI_SPLIT_LENGTH +
                     MI_DYN_DELETE_BLOCK_HEADER + 1);
      tmp = (info->s->base.pack_reclength +
                   _my_calc_total_blob_length(info, record) + extra);
      rec_buff = (uchar *)my_malloc(mi_key_memory_record_buffer, tmp,
                                    MYF(0));
      if (!rec_buff) {
      ...
      reclength = _mi_rec_pack(info, rec_buff +
                               ALIGN_SIZE(MI_MAX_DYN_BLOCK_HEADER), record);
      assert(reclength <= tmp);
      record = rec_buff + ALIGN_SIZE(MI_MAX_DYN_BLOCK_HEADER);
    } else {/* Checking dynamic record */
      /* Getting size of dynamic record */
      /* Referenced in _mi_write_dynamic_record() */
      reclength = _mi_rec_pack(info, info->rec_buff, record);
    }

    /* Checks if this record is long(< 65520) */
    int long_length = reclength >= (65520 - 3) ? 1 : 0; 
    if (reclength + 3 < info->s->base.min_block_length)
      size = info->s->base.min_block_length;
    else
      size = long_length == 1 ? 65520
                              : ((reclength + 3) + MI_DYN_ALIGN_SIZE - 1)
                              & (~(ulong)(MI_DYN_ALIGN_SIZE - 1));
    
    info->packed_length = reclength;
  } else {
    /* Getting size of static record */
    size = info->s->base.reclength;
  }

  /* If this record cannot to be inserted into this page */
  if (__is_next_page(filepos, size))
    filepos = adjust_offset(info, filepos, size);
#endif /* __ZICIO_PAGINATION */
