...
int _mi_mem_read_cache(IO_CACHE *info, uchar *buff, my_off_t pos, uint length,
                   int flag, uchar *data_page) {
  uint read_length, in_buff_length;
  my_off_t offset;
  uchar *in_buff_pos;
  DBUG_TRACE;
  ...
  if (pos < info->pos_in_file) {
    ...
    if ((my_off_t)read_length > (my_off_t)(info->pos_in_file - pos))
      read_length = (uint)(info->pos_in_file - pos);
    info->seek_not_done = true;
    memcpy(buff, data_page + __get_offset(pos), read_length);
    if (!(length -= read_length)) return 0;
    pos += read_length;
    buff += read_length;
  }
  ...
  if (flag & READING_NEXT) {
    ...
    info->seek_not_done = true;
    memcpy(buff, data_page + __get_offset(pos), length);
    return 0;
  }
  if (!(flag & READING_HEADER) || (int)read_length == -1 ||
      read_length + in_buff_length < 3) {
