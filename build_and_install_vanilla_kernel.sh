...
sed -Ei 's/EXTRAVERSION =-zicio/EXTRAVERSION =/' Makefile
# Configure kernel
...
if [ -z "$(awk -F\' '/menuentry / {print $2}' /boot/grub/grub.cfg | grep -m 1 'Ubuntu, with Linux 5.12.0')" ]; then
    printf "Cannot find vanilla 5.12 kernel. Please install the kernel manually.\n"
    exit 1
fi

printf "vanilla kernel is installed. To boot into vanilla kernel, please run:\n"
printf "    sudo grub-reboot \"Advanced options for Ubuntu>Ubuntu, with Linux 5.12.0\"\n"
printf "    sudo reboot\n"
