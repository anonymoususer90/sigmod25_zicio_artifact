...
#ifdef CONFIG_ZICIO
void *__zicio_ext_inode_hdr(struct inode *inode)
{
	return ext_inode_hdr(inode);
}

struct buffer_head*
zicio_read_extent_tree_block(void *inode, unsigned long long pblk,
			int depth, int flags)
{
	return read_extent_tree_block(inode, pblk, depth, flags);
}

void *ZICIO_EXT4_SB(struct super_block *sb) {
	return EXT4_SB(sb);
}

int zicio_ext4_forced_shutdown(void *sbi) {
	return ext4_forced_shutdown(sbi);
}
#endif
...
#ifdef CONFIG_ZICIO
void zicio_ext4_drop_refs(void *path)
{
	ext4_ext_drop_refs(path);
}
#endif
...
#ifdef CONFIG_ZICIO
/**
 * @brief Set nvme_cmd_infos of a single file.
 * 
 * @param desc zicio_descriptor
 * @param fd_idx index of fd of the current file in the batches_array
 * 				 (== index in the current zicio_rfile)
 * @param file_inode inode of the current file
 * @param offset_in_hugepage offset of a new nvme_cmd_info in a hugepage
 * 
 * @return 0 in normal case, otherwise 1
 */
int zicio_set_nvme_cmd_infos(void *desc, int fd_idx, struct inode *file_inode,
							 unsigned int *offset_in_hugepage) {
	zicio_notify_descriptor *zicio_notify_desc = (zicio_notify_descriptor*)desc;
	unsigned int nr_batches = zicio_notify_desc->nr_batches[fd_idx];
	ext4_lblk_t *batches = zicio_notify_desc->batches_array[fd_idx];
	ext4_lblk_t start, end, ee_block, start_offset, ext_len;
	ext4_fsblk_t fsblk;
	struct ext4_ext_path *path = NULL;
	short int depth, ppos;
	struct ext4_extent *ex = NULL;
	int i, left_ext, left_hugepage, left, length, chunk_idx, elem_idx, ret = 0;

	inode_lock_shared(file_inode);

	depth = ext_depth(file_inode);
	ppos = depth;
	...
	for (i = 0; i < nr_batches; i++) {
		...
		/* the first batch of the current file, or out of the current extent */
		if (unlikely(path == NULL || start >= ee_block + ext_len ||
					 end < ee_block)) {
reset_ext:
			path = ext4_find_extent(file_inode, start, &path,
									EXT4_EX_FORCE_CACHE);
			...
			/* get extent */
			if (unlikely(path[ppos].p_ext == NULL)) {
			...
			ee_block = (ext4_lblk_t)le32_to_cpu(ex->ee_block);
			ext_len = (ext4_lblk_t)ext4_ext_get_actual_len(ex);
		}

		/*
		 * Get distance between ee_block and start, and adjust fsblk and
		 * left_ext with it.
		 */
		start_offset = start - ee_block;
		fsblk = path[ppos].p_block + start_offset;
		left_ext = ext_len - start_offset;
		BUG_ON(left_ext == 0);

		/*
		 * The zicio stores batches in ascending order of block number, so we
		 * don't expect to search the left extent.
		 */
		BUG_ON(start < ee_block);

		/* set nvme_cmd_infos of the current batch */
		while (left > 0) {
			length = left;
			left_hugepage =
				ZICIO_NVME_CMD_INFO_MAX_NUMBER - *offset_in_hugepage;
			BUG_ON(left_hugepage == 0);

			/* adjust the length of this nvme_cmd_info */
			if (length > ZICIO_NVME_CMD_INFO_MAX_LENGTH)
				length = ZICIO_NVME_CMD_INFO_MAX_LENGTH;

			if (unlikely(length > left_hugepage))
				length = left_hugepage;

			if (unlikely(length > left_ext))
				length = left_ext;

			/* record start offset */
			if (unlikely(*offset_in_hugepage == 0)) {
				BUG_ON(zicio_notify_desc->nr_nvme_cmd_info_start_offsets >=
					   ZICIO_NVME_CMD_INFO_START_OFFSETS_CAPACITY);

				chunk_idx = zicio_notify_desc->nr_nvme_cmd_info_start_offsets /
							ZICIO_NVME_CMD_INFO_START_OFFSET_PER_CHUNK;
				elem_idx = zicio_notify_desc->nr_nvme_cmd_info_start_offsets %
						   ZICIO_NVME_CMD_INFO_START_OFFSET_PER_CHUNK;
				zicio_notify_desc->nr_nvme_cmd_info_start_offsets += 1;

				/* the first of the chunk (new nvme_cmd_info_start_offset) */
				if (elem_idx == 0) {
					unsigned int *nvme_cmd_info_start_offsets =
						(unsigned int*)kmalloc(ZICIO_NVME_CMD_INFO_START_OFFSET_CHUNK_SIZE,
											   GFP_KERNEL|__GFP_ZERO);
					if (unlikely(!nvme_cmd_info_start_offsets)) {
						ret = 1;
						goto cleanup;
					}

					zicio_notify_desc->nvme_cmd_info_start_offsets[chunk_idx] =
						nvme_cmd_info_start_offsets;
				}

				zicio_notify_desc->nvme_cmd_info_start_offsets[chunk_idx][elem_idx] =
					zicio_notify_desc->nr_nvme_cmd_infos;
			}

			BUG_ON(zicio_notify_desc->nr_nvme_cmd_infos >=
				   ZICIO_NVME_CMD_INFOS_CAPACITY);

			chunk_idx = zicio_notify_desc->nr_nvme_cmd_infos /
						ZICIO_NVME_CMD_INFO_PER_CHUNK;
			elem_idx = zicio_notify_desc->nr_nvme_cmd_infos %
					   ZICIO_NVME_CMD_INFO_PER_CHUNK;
			zicio_notify_desc->nr_nvme_cmd_infos += 1;

			/* the first of the chunk (new nvme_cmd_info) */
			if (elem_idx == 0) {
				zicio_nvme_cmd_info *nvme_cmd_infos =
					(zicio_nvme_cmd_info*)kmalloc(ZICIO_NVME_CMD_INFO_CHUNK_SIZE,
												  GFP_KERNEL|__GFP_ZERO);
				if (unlikely(!nvme_cmd_infos)) {
					ret = 1;
					goto cleanup;
				}

				zicio_notify_desc->nvme_cmd_infos[chunk_idx] = nvme_cmd_infos;
			}

			zicio_notify_desc->nvme_cmd_infos[chunk_idx][elem_idx] =
				zicio_get_nvme_cmd_info(fsblk, length - 1, fd_idx);

			/* ready for the next nvme_cmd_info */
			start += length;
			left -= length;
			*offset_in_hugepage = (*offset_in_hugepage + length) %
								   ZICIO_NVME_CMD_INFO_MAX_NUMBER;

			/* check for the next extent */
			if (unlikely(length == left_ext && left > 0)) {
				goto reset_ext;
			} else {
				fsblk += length;
				left_ext -= length;
