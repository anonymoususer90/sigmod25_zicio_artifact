...
ssize_t zicio_iov_iter_get_pages(struct iov_iter *i, struct page **pages,
			size_t maxsize, unsigned max_pages, size_t *start)
{
	size_t len;
	int n, res, npage;

	*start = (unsigned long)i->iov->iov_base % ZICIO_PAGE_SIZE;
	...
	n = DIV_ROUND_UP(len, ZICIO_PAGE_SIZE);
	res = i->count >> ZICIO_PAGE_SHIFT;

	for (npage = 0 ; npage < res ; npage++) {
		pages[npage] = virt_to_page((unsigned long)i->iov->iov_base +
					ZICIO_PAGE_SIZE * npage);
	}

	return res * PAGE_SIZE - *start;
}
EXPORT_SYMBOL(zicio_iov_iter_get_pages);
