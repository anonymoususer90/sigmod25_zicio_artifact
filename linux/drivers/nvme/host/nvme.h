...
static inline void zicio_dump_nvme_request(struct nvme_request * nvm_req)
{
	printk(KERN_WARNING "[ZicIO Message] cmd ptr : %p\n", nvm_req->cmd);
	printk(KERN_WARNING "[ZicIO Message] result size : %ld\n",
				sizeof(nvm_req->result));
	printk(KERN_WARNING "[ZicIO Message] genctr : %hhu\n",nvm_req->genctr);
	printk(KERN_WARNING "[ZicIO Message] retries : %hhu\n", nvm_req->retries);
	printk(KERN_WARNING "[ZicIO Message] flags : %hhu\n", nvm_req->flags);
	printk(KERN_WARNING "[ZicIO Message] flags : %hu\n", nvm_req->status);
}
#endif
...
#if (CONFIG_ZICIO_DEBUG_LEVEL >= 2)
		zicio_dump_nvme_request(nvme_req(rq));
#endif
		BUG_ON(true);
...
/* This disposition function is exported for zicio */
